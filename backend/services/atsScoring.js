const { detectSections, hasMeaningfulContent } = require('./resumeParser');
const {
  extractJDKeywords,
  extractKnownSkills,
  analyzeKeywordMatch,
...
function relevanceRatio(block, jdKeywords) {
  ...
  let matched = 0;
  for (const kw of all) {
    if (buildBoundaryRegex(kw).test(normalized)) matched += 1;
...
function scoreKeywordMatch(resumeText, jdText) {
  if (!hasMeaningfulContent(jdText)) {
    // A blank, single-word, or otherwise too-short JD isn't a real job
    // description to match against — be honest about that with a neutral
    // baseline instead of producing misleadingly specific numbers like "0%".
...
function scoreFormatting(resumeText) {
  ...
  // Skip the header block (name, tagline, contact line, links line) — a
  // pipe-separated header like "phone | location | email" is standard,
  // legitimate formatting, not a table. Only count pipe-grid lines in the
  // body, and require considerably more of them before suspecting an actual
  // table (a genuine pasted table has many rows, not two or three).
  const bodyLines = resumeText.split('\n').slice(5);
  const pipeGridLines = bodyLines.filter((l) => (l.match(/\|/g) || []).length >= 2).length;
  if (pipeGridLines >= 5) {
    score -= 25;
...
function scoreGrammar(resumeText) {
  ...
  // Repetition of domain/technical terms (e.g. "test", "testing", "defect"
  // on a QA resume) is expected and actually HELPS ATS keyword matching —
  // it shouldn't be scored the same as lazy prose repetition. Lighter
  // weight + higher threshold (see findRepeatedWords) reflects that.
  score -= Math.min(12, repeated.length * 2);
  score -= Math.min(15, buzzwords.length * 5);
  // Generic readability formulas (Flesch-style) are calibrated for prose,
  // not bullet-heavy technical resumes full of jargon, acronyms, and tool
  // names — those naturally score "low" without being poorly written.
  if (readability < 20) score -= 12;
  else if (readability < 35) score -= 6;
